...
```
You describe it → Claude plans it → You review/edit → Claude codes it → TypeScript validated → Preview plays inline
```
...
After writing the scene file, the system runs `npx tsc --noEmit` against the Remotion project. If it finds errors, Opus takes a crack at fixing them automatically. The UI shows a step-by-step progress: writing file, validating TypeScript, auto-fixing. If it still fails, you see the errors in a structured format with error code, file, line number, not just a wall of red text.
