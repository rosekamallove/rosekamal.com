...
### The API was not what I thought it was

I built the integration against the EmailBison docs. The base URL I had was wrong. The endpoint for sending a reply was wrong. The endpoint for fetching thread history was wrong.

The send endpoint I assumed: `POST /threads/{threadId}/reply`

The actual endpoint: `POST /replies/{replyId}/reply`

Not the same object. Not the same ID. And the ID I had been calling `threadId` throughout the codebase was actually `scheduled_email.id` from the webhook payload — a different identifier entirely from the `reply.id` that the send API actually wants.

I found the correct base URL by reading an unrelated export script that had been written months earlier against the live API. The docs were wrong. The script was right.

First real reply came in. Got a 422: `sender_email_id required`. Turns out the API needs `reply_all: true` in the body to infer the sender — otherwise it expects you to specify the sender email ID explicitly. One field, not in the docs, required to make it work.

**Lesson:** Integration tests against the real API before you think you're done. Not mocks. The real thing.
...
In production, the Slack reaction fired but couldn't find the message — silently skipped. I only noticed when I watched the Slack channel and saw no reaction appear.
...
Added a threshold: below 70% confidence, route to Tom regardless of the classified intent. The model is uncertain — a human should decide.
...
**Test the full pipeline against the real API before writing any logic.** The normalisation bugs, the endpoint bugs, the ID bugs — all of them would have surfaced in the first 10 minutes of real testing. Instead I found them one by one in production.
